...
for i = 1:numarValoriSelectie
    valoriSelectie(i) = simuleazaCauchy();
end
...
densitateCauchy = (1/pi) .* (1./(1 + valoriSelectieNoi.^2));

% Metoda de verificare cu Histograma
close all
hold on
bar(centre,numarValoriInterval,1,'y');
plot(valoriSelectieNoi,densitateCauchy,'r');
hold off
...
probabilitati(1) = functieRepartitieCauchy(a(1));
for i = 2:numarOptimDreptunghiuri - 1
    probabilitati(i) = functieRepartitieCauchy(a(i)) - functieRepartitieCauchy(a(i-1));
end
probabilitati(numarOptimDreptunghiuri) = 1 - functieRepartitieCauchy(a(numarOptimDreptunghiuri-1));
